...
@interface runWaterViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    int _offset;//分页查询起始值
    int _page_size;//分页大小
...
@implementation runWaterViewController
...
- (void)viewDidLoad {
    ...
    self.navigationItem.titleView = [[navLabel alloc] initWithFrame:lableFrame titile:@"流水"];
...
{
    historyAndMoneyTableViewCell *cell = [[[NSBundle mainBundle] loadNibNamed:@"historyAndMoneyTableViewCell" owner:self options:nil] objectAtIndex:2];
    ...
    if (_dicData.count>0) {
        ...
        [cell balanceSheetSetModel:model];
...
- (void)getSendOrderList{
    ...
    NSArray *dataArray = [[NSArray alloc] initWithObjects:[NSString stringWithFormat:@"%d",_offset],[NSString stringWithFormat:@"%d",_page_size],[NSString stringWithFormat:@"%d",2],nil];
    ...
    inModel.type = [NSString stringWithFormat:@"%d",2];
