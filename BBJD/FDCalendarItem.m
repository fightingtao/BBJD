...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
  ...
  NSDateFormatter  *dateFormatter=[[NSDateFormatter alloc] init];
    ...
    int time=(int)  [selectedDate timeIntervalSinceDate:date2]; //登录的时间查
    DLog(@"时间%d",time);
    if (time<0) {
        [[iToast makeText:@"请选择今天之后的某一天"]show];
        return;
    }
    
    NSDictionary *dict = [[NSDictionary alloc]initWithObjectsAndKeys:selectedDate,@"chuanzhi", nil];
    
    [[NSNotificationCenter defaultCenter]postNotificationName:@"selectedDate"  object:nil userInfo:dict];
}
