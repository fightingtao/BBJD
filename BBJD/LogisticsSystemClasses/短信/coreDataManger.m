    ...
    NSArray *arr = [context executeFetchRequest:request error:nil];
     for (NSManagedObject *object in arr) {
        ...
        NSString *phone = [object valueForKey:@"phone"];
         ...
         NSTimeInterval _interval=([time doubleValue])/1000+8*3600 ;
         NSDate *last=[NSDate dateWithTimeIntervalSince1970:_interval ];
         ...
         // 对比时间差
         NSDateComponents *dateCom = [calendar components:unit fromDate:now  toDate:last options:0];
         
        
         
         if (dateCom.day>7){
             //创建谓词
             NSPredicate *predicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"phone CONTAINS '%@'",phone]];//like
             //将谓词赋予请求
         ...
         NSLog(@"耗时%ld天%ld小时%ld分  ",(long)dateCom.day,(long)dateCom.hour,(long)dateCom.hour);
...
-(void)changeCoreDataMsg:(NSString *)orderId kind:(NSString *)kind changeText:(NSString *)changeText isSend:(NSString *)send{
    ...
    request.entity = entity;
    //创建谓词
    NSPredicate *predicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"phone like '%@' AND recipient_mobile like '%@'",orderId,userphone]];
    //将谓词赋予请求
    request.predicate = predicate;
    ...
    //    NSLog(@"%@ %@",[object valueForKey:@"phone"],[object valueForKey:@"age"]);
        ...
        if ([kind isEqualToString:@"time"]) {
            NSManagedObject *obj=object;
            [obj  setValue:changeText forKey:@"time"];//已读
...
>>>>>>> 4ab4426828149e0ad1f773d811fbd7720a0ff84c
     ...
     //  NSString *phone = [object valueForKey:@"phone"];
       ...
       if (dateCom.day>7){
           //创建谓词
           NSPredicate *predicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"time CONTAINS '%@'",time]];//like
           //将谓词赋予请求
           request.predicate = predicate;
